...
function SecureInclude($_){
	return @require_once ReturnUnset($_);}

\spl_autoload_register(callback:function(string $_):void{
	if(\is_file($_=\implode(DirSeparator,[Root,\strtolower($_).'.php'])))
		SecureInclude($_);});

foreach(\iterator_to_array(new \RegexIterator(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator(\implode(DirSeparator,[Root,'.function']))),'/\.php$/')) as $_)
	SecureInclude($_);
